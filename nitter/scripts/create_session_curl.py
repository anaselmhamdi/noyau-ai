...
def submit_password(session, flow_token, headers, guest_token, password):
    ...
    needs_2fa = any(
        s.get("subtask_id") == "LoginTwoFactorAuthChallenge" for s in data.get("subtasks", [])
    )
    if needs_2fa:
...
def submit_js_instrumentation(session, flow_token, headers, guest_token):
    ...
    flow_token, _ = make_request(
        session, headers, flow_token, subtask, "Submitting JS instrumentation"
    )
    return flow_token
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python3 create_session_curl.py username password [totp_seed] [--append sessions.jsonl]",
            file=sys.stderr,
        )
        sys.exit(1)
